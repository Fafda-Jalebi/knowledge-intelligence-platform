...
async def get_embedding_providers(user_id: int = Depends(get_current_user_id)) -> list[dict]:
...
async def get_llm_providers(user_id: int = Depends(get_current_user_id)) -> list[dict]:
...
async def get_rerankers(user_id: int = Depends(get_current_user_id)) -> list[dict]:
...
async def get_vector_stores(user_id: int = Depends(get_current_user_id)) -> list[dict]:
...
async def get_keyword_indexes(user_id: int = Depends(get_current_user_id)) -> list[dict]:
